...
`;
  ...
  function layout(circuit) {
    ...
    // Assign positions — depth 0 centered, deeper columns pulled toward inputs
    const positions = {};
    ...
    // Position gate columns left to right, pulling toward input positions
    for (let d = 1; d <= maxDepth; d++) {
      if (!columns[d]) continue;
      const x = MARGIN_X + d * COL_GAP;
      const col = columns[d];

      // Compute ideal Y: average of input node positions
      col.forEach(n => {
        const inputYs = n.inputs
          .map(id => positions[id] ? positions[id].y : null)
          .filter(y => y !== null);
        n._idealY = inputYs.length
          ? inputYs.reduce((a, b) => a + b, 0) / inputYs.length
          : totalH / 2;
      });

      // Place each node at its ideal Y (order stays from barycenter sort)
      col.forEach(n => {
        positions[n.id] = { x: x, y: n._idealY };
      });

      // Resolve overlaps top-to-bottom: enforce ROW_GAP minimum spacing
      ...
      // Ensure nothing above the top margin
      ...
      // Settle: pull each node toward its ideal Y without violating spacing.
      // Two passes — backward then forward — to distribute slack evenly.
      for (let pass = 0; pass < 2; pass++) {
        for (let i = col.length - 1; i >= 0; i--) {
          const n = col[i];
          const target = n._idealY;
          let lo = MARGIN_Y + nodeSizes[n.id].h / 2;
          let hi = totalH - MARGIN_Y - nodeSizes[n.id].h / 2;
          if (i > 0) {
            const prev = col[i - 1];
    ...
    // Recompute totalH if layout grew taller
    let maxBottomY = 0;
    for (const d in columns) {
      const col = columns[d];
  ...
  function render(circuit, options) {
    ...
    // Group edges by destination depth — vertical segments route in the gap
    // just before the destination column, so all wires entering a column
    // get unique track positions within the same gap.
    var gapWires = {};
    edges.forEach(function(e) {
      ...
      var key = toNode.depth;
      if (!gapWires[key]) gapWires[key] = [];
      gapWires[key].push({ edge: e, fromP: fromP, toP: toP });
    });

    // For each destination gap, assign unique track x-offsets
    for (var gapKey in gapWires) {
      var wires = gapWires[gapKey];
      var destDepth = parseInt(gapKey);

      // Track region: centered in the gap just before the destination column
      var gapCenterX = MARGIN_X + (destDepth - 0.5) * COL_GAP;

      // Sort by vertical midpoint to minimize visual crossings
      wires.sort(function(a, b) {
        return (a.fromP.y + a.toP.y) / 2 - (b.fromP.y + b.toP.y) / 2;
      });
      var trackSpacing = 8;
      ...
      wires.forEach(function(w, idx) {
        ...
        if (Math.abs(fromP.y - toP.y) < 1) {
          ...
          // Z-shaped routing with unique track in the destination gap
          var midX = gapCenterX + startOffset + idx * trackSpacing;
